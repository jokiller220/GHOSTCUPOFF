-- Ghost Cup full database schema
-- Use this script to create the core tournament schema in Supabase.
-- Note: Supabase Auth user creation is handled by Supabase Auth; profile rows can be inserted after auth users exist.
...
-- Create the Supabase Auth users first with these emails and password.
-- Then insert matching profiles using the auth.user ids.
-- Credentials:
--   joakim@ghostcup.com / VICTOIRE2026
--   david@ghostcup.com / VICTOIRE2026

-- Example profile insertion after auth user creation:
-- INSERT INTO profiles (id, cod_username, real_name, role)
-- VALUES
--   ('<JOAKIM_AUTH_USER_ID>', 'joakim', 'Joakim', 'admin'),
--   ('<DAVID_AUTH_USER_ID>', 'david', 'David', 'admin');
